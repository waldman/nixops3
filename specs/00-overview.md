...
NixOpS3 is a pull-based NixOS configuration management system. Each managed machine runs a daemon (`nixops3 --daemon`) that periodically fetches its NixOS configuration from an S3 bucket and applies it via `nixos-rebuild switch`.
...
| canary.txt parsing | plain text, line-by-line (no YAML) |
